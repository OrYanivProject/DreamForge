            ...
            if ((isSameCell && !this.isPartialExport) || isNullOrUndefined(startCell)) {
                ...
                while (nextBlock) {
                    ...
                    if (isDifferentSection) {
                        ...
                        this.document[sectionsProperty[this.keywordIndex]].push(section);
                        var paragraph_1 = this.createParagraph(nextBlock);
                        section[blocksProperty[this.keywordIndex]].push(paragraph_1);
                        lastBlock = nextBlock;
                        nextBlock = this.writeParagraph(lastBlock, paragraph_1, section[blocksProperty[this.keywordIndex]], line.indexInOwner, startOffset);
                        continue;
                    }
                    lastBlock = nextBlock;
    ...
    SfdtExport.prototype.writeParagraph = function (paragraphWidget, paragraph, blocks, lineIndex, start) {
        ...
        while (next instanceof ParagraphWidget) {
            if (this.writeLines(next, lineIndex, start, paragraph[inlinesProperty[this.keywordIndex]])) {
                if (this.endLine === next.lastChild && this.endOffset === this.owner.selection.getLineLength(next.lastChild) + 1 && next.paragraphFormat.listFormat.listId === -1) {
